...
% ismus = ismember(si,find(mus_id==1));
% mus_idx = find(ismus);
% isnonmus = ismember(si,find(mus_id==0));
% nonmus_idx = find(isnonmus);
% for i = 1:M
%    X_zero_mean_rows(i,mus_idx) = X(i,mus_idx) - mean(X(i,mus_idx));
%    X_zero_mean_rows(i,nonmus_idx) = X(i,nonmus_idx) - mean(X(i,nonmus_idx));
% end
