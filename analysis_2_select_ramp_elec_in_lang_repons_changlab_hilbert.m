3% selecting langauge responsive electrodes and add them to the subject
% info. 
...
% step 1: extract electrodes with siginificant language response
    ...
    sentence_gamma_band_ave_envelope=cellfun(@(x) x(1:8),{sentences.signal_ave_hilbert_downsample_parsed},'UniformOutput',false);
...
for i=1:length(ramp_electrodes)
    ...
    channel_response=double(squeeze(electrode_with_ramp_across_sessions(i,:,:)));
...
for i=2:2:length(d)
    ...
    % sentence
    sentences=[data{sentence_trial_index}];
    sentence_gamma_band_envelope=cellfun(@(x) x(1:8),{sentences.signal_hilbert_downsample_parsed},'UniformOutput',false);
    % creat a cell with wordposition(row)*time in trial(column) structure
    sentence_gamma_band_envelope=[sentence_gamma_band_envelope{:,:}];
    % convert the gamma into a tensor of shape : channel*time in trial*
    % trial number
    ...
    nonwords_gamma_band_envelope=cellfun(@(x) x(1:8),{nonwordss.signal_hilbert_downsample_parsed},'UniformOutput',false);
    % creat a cell with wordposition(row)*time in trial(column) structure
    ...
    jabberwocky_gamma_band_envelope=cellfun(@(x) x(1:8),{jabberwockys.signal_hilbert_downsample_parsed},'UniformOutput',false);
    % creat a cell with wordposition(row)*time in trial(column) structure
    ...
    words_gamma_band_envelope=cellfun(@(x) x(1:8),{wordss.signal_hilbert_downsample_parsed},'UniformOutput',false);
    % creat a cell with wordposition(row)*time in trial(column) structure
