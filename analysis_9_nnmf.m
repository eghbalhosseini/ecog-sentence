...
all_electrode_reconstruct_means={[],[],[]};
...
for i=1:length(language_electrode_num)
    ...
    set(a,'XTick',[1:8])
    e.Color=[0,0,0];
    all_electrode_reconstruct_means{1,kk}=[all_electrode_reconstruct_means{1,kk};mean(electrode_reconstructed,1)];
    end 
    
    for kk=1:size(H_data,1)
    a=subplot(4,3,[9+kk]);
    pattern_reconstructed=W_pattern(:,kk)*H_pattern(kk,:);
    h=scatter(perturbed_word_position(:),pattern_reconstructed(:),4);
    set(h,'CData',colors(2,:),'MarkerFacecolor',colors(1,:),'MarkerFaceAlpha',.3)
    hold on
    e=plot(mean(word_position,1),mean(pattern_reconstructed,1),...
        '-s','MarkerSize',5,'MarkerEdgeColor','k','MarkerFaceColor','k','LineWidth',2,'color','k');
    xlim([min(min(word_position))-.5,max(max(word_position))+.5])
    set(a,'XTick',[1:8])
    e.Color=[0,0,0];
...
for i=1:3
subplot(3,1,i)
nnmf_factor=all_electrode_reconstruct_means{1,i};
word_position=repmat(1:size(nnmf_factor,2),[size(nnmf_factor,1),1]);
h=scatter(word_position(:),nnmf_factor(:),4);
set(h,'CData',colors(6,:),'MarkerFacecolor',colors(1,:),'MarkerFaceAlpha',.3)
hold on
e=plot(mean(word_position,1),mean(nnmf_factor,1),...
    '-s','MarkerSize',5,'MarkerEdgeColor','k','MarkerFaceColor','k','LineWidth',2,'color','k');
xlim([min(min(word_position))-.5,max(max(word_position))+.5])
set(a,'XTick',[1:8])
e.Color=[0,0,0];
all_electrode_reconstruct_means{1,kk}=[all_electrode_reconstruct_means{1,kk};mean(electrode_reconstructed,1)];
y_quantile=quantile(nnmf_factor(:),50);
ylim([y_quantile(1),y_quantile(end)]);
end 
